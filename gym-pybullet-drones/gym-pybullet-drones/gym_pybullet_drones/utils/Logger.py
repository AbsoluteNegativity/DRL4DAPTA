...
class Logger(object):
    ...
    def __init__(self,
        ...
        self.TRAJ_STEPS = 1440
        self.INIT_XYZSforA=[0.0,0.0,0.5]
        self.MAX_LIN_VEL_XY=5.0
        self.MAX_LIN_VEL_Z=2.0
        self.PERIOD=30

        self.MAX_XY=self.MAX_LIN_VEL_XY*self.PERIOD
        self.MAX_Z=self.MAX_LIN_VEL_Z*self.PERIOD
        self.TARGET_POSITION = np.array([[55*i/self.TRAJ_STEPS+self.INIT_XYZSforA[0],\
            10*np.sin(5*i/self.TRAJ_STEPS)+self.INIT_XYZSforA[1],\
                25.05*i/self.TRAJ_STEPS+self.INIT_XYZSforA[2]] for i in range(self.TRAJ_STEPS)])
    ...
    def plot(self, pwm=False):
        ...
        for j in range(self.NUM_DRONES):
            # axs[row, col].plot(t, self.states[j, 0, :]*self.MAX_XY, label="drone_"+str(j))
            axs[row, col].plot(t, self.TARGET_POSITION[:, 0], label="drone_x")
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel('x (m)')

        row = 1
        for j in range(self.NUM_DRONES):
            # axs[row, col].plot(t, self.states[j, 1, :]*self.MAX_XY, label="drone_"+str(j))
            axs[row, col].plot(t, self.TARGET_POSITION[:, 1], label="drone_x")
        axs[row, col].set_xlabel('time')
        axs[row, col].set_ylabel('y (m)')

        row = 2
        for j in range(self.NUM_DRONES):
            # axs[row, col].plot(t, self.states[j, 2, :]*self.MAX_Z, label="drone_"+str(j))
            axs[row, col].plot(t, self.TARGET_POSITION[:, 2], label="drone_x")
        axs[row, col].set_xlabel('time')
